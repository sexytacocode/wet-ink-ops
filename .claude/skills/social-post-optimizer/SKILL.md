...
description: >
  ...
  told otherwise. Use before writing social copy manually. This skill produces text copy
  only — it does not build Canva assets; for the visual design itself use instagram-carousel
  or instagram-reels.
---
...
> **Tool-name portability:** Tool names vary by surface — in Claude Code/agent mode use `WebFetch` (instead of `web_fetch`) and session-transcript search instead of `conversation_search`.
...
Base voice: apply the wet-ink-voice skill. Social adaptation: shorter sentences are fine here — that skill's anti-choppiness rule targets editorial prose, not captions. Keep it human, insider, never AI-sounding.
...
> **Specs current as of Apr 2025** — if a post gets truncated or rejected, re-verify the platform limit rather than trusting these numbers.
...
- **YouTube hashtags must be SFW-safe — never #AdultIndustry, #SexWork, or variants; YouTube is a strike surface. The Instagram hashtag pool is different and may include industry tags.**
...
- If publishing via the Instagram MCP (no first-comment support), append the hashtags to the caption end instead
...
HASHTAGS: #WetInkMag #OpheliaFae #CreatorEconomy #Interview
...
FIRST COMMENT HASHTAGS:
#WetInkMag #OpheliaFae #CreatorEconomy #AVN #SexWorkIsWork #AdultPerformer #ContentCreator
...
CAPTION:
Ophelia Fae quit corporate and became an AVN-nominated performer in under a year. What's your excuse? Full guide on wetinkmag.com #WetInkMag #OpheliaFae #CreatorEconomy (167 chars)
